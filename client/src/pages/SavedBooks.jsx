...
/*`SearchBooks.jsx`:
  * Use the Apollo `useMutation()` Hook to execute the `SAVE_BOOK`
 mutation in the `handleSaveBook()` function instead of the `saveBook()`
  function imported from the `API` file.
  
  * Make sure you keep the logic for saving the book's ID to state in 
  the `try...catch` block! */
...
import { REMOVE_BOOK, SAVE_BOOK} from '../utils/mutations'; // **import the REMOVE_BOOK mutation
...
const SavedBooks = () => {
  ...
  //const [userData, setUserData] = useState({});
  const { loading, userData } = useQuery( GET_ME); // Instead, use the `useQuery()` Hook to execute the `GET_ME` query on load and save it to a variable named `userData`.
  const [removeBook, { error }] = useMutation(REMOVE_BOOK); //** Use the `useMutation()` Hook to execute the `REMOVE_BOOK` mutation in the `handleDeleteBook()` function instead of the `deleteBook()`
